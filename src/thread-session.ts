...
export class ThreadSession {
  ...
  static async create(params: ThreadSessionCreateParams): Promise<ThreadSession> {
    ...
    const { session } = await createAgentSession({
      ...
      // Pi 0.84 expects tool names here, not the Tool[] returned by
      // createCodingTools(). The default coding set includes bash.
    ...
    // Pi 0.84 replaced AgentSession.modelRegistry with modelRuntime.
    // getModels() is the synchronous model catalogue used during startup.
  ...
  /**
   * Submit a user message using Pi's steer semantics by default.
   * While the agent is running, Pi delivers it after the current tool call
   * and before the next model request. Otherwise it enters the normal queue.
   */
  async submit(text: string, options?: { images?: ImageContent[] }): Promise<void> {
    this.lastActivity = new Date();
    if (this.isStreaming) {
      await this._agentSession.steer(text, options?.images);
      return;
    }
    this.enqueue(() => this.prompt(text, options));
  }
  ...
  get sessionName(): string | undefined {
    return this._agentSession.sessionName;
  }

  get modelRuntime(): AgentSession["modelRuntime"] {
    return this._agentSession.modelRuntime;
  }
  ...
  async setModel(modelName: string): Promise<void> {
    const runtime = this._agentSession.modelRuntime;
