...
export interface CommandContext {
  ...
  /** Best-effort sync to Slack Assistant thread title. */
  setThreadTitle?: (title: string) => Promise<void>;
}
...
const handlers: Record<string, CommandHandler> = {
  async help(ctx) {
    const lines = [
      ...
      "`!title [direction]` — Regenerate the AI title from the full conversation and sync Slack",
  ...
  async title(ctx, args) {
    if (!ctx.session) {
      await reply(ctx, "No active session.");
      return;
    }
    try {
      await ctx.session.prompt(args.trim() ? `/agent-title ${args.trim()}` : "/agent-title");
      const title = ctx.session.sessionName;
      if (!title) {
        await reply(ctx, "❌ Pi did not produce a title. Check that pi-topic-title is loaded.");
        return;
      }
      if (ctx.setThreadTitle) await ctx.setThreadTitle(title);
      await reply(ctx, `✅ Title updated: *${title}*`);
    } catch (err) {
      await reply(ctx, `❌ Failed to update title: ${err instanceof Error ? err.message : String(err)}`);
    }
  },
